...
def plot_figure(all_x, all_y, fig_save_name, marker_num, marker_size):
    ...
    context = draw_base.Context(xlabel, ylabel1, ylabel2, file2save=fig_save_name, y1Lim=yLim, xLim=xLim,
                                ...
                                LWidth=LWidth, y2Ticks=y2Ticks, y2TickLabels=y2TickLabels, FH=9, FW=6)
...
def main(opts):
    ...
    fig_save_name = os.path.join(outDir + 'acc_cmp_{}.png'.format(opts.trace_num))
    data_save_name = os.path.join(outDir, 'acc_cmp_{}.npz'.format(opts.trace_num))
...
def parseArgs(argv):
    ...
    parser.add_argument('-mn', '--marker_num', type=int, default=5, help='')
    parser.add_argument('-ms', '--marker_size', type=int, default=16, help='')
