...
def main(opts):
    ...
    X_attack = X_attack.reshape(-1, 1000, 1)

    # Get the input shape of the model
    input_shape = model.input_shape  # This may return (None, num_features)

    # Check the shape of X_attack
    x_shape = X_attack.shape

    # Determine the new shape for X_attack
    new_shape = (x_shape[0], input_shape[1])  # Preserve the batch size, update the feature dimension

    # Reshape X_attack
    Reshaped_X_attack = X_attack.reshape(new_shape)
