...
def distance(power_trace,power_trace2,n):
    indx=min(len(power_trace), len(power_trace2))
    a_avg = np.mean(power_trace[:140000], axis = 0)
    b_avg = np.mean(power_trace2[:n], axis = 0)
    distance=(a_avg-b_avg)
    net_distance = sum(distance)
    net_distance
    avg=net_distance/len(distance)
    modified_trace=[]
    for i in range(n):
        power_trace2[i]
        t=[]
        for j in range(5000):
            t.append(power_trace2[i][j]+distance[j])
...
def main(opts):
    ...
    modified_trace=distance(source_traces,target_traces,opts.test_trace_num)
