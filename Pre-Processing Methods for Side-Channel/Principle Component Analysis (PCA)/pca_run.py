...
def pca_main(traces, n_components):
...
def pca2(traces,pca):
    pca_traces2 = pca.transform(traces)
    return pca_traces2
...
def main(opts):
    ...
    n_classes = 256
    pca = pca_main(traces, n_classes)
    pca_traces2=pca2(traces2,pca)
    os.makedirs(opts.output, exist_ok=True)
    modelDir = os.path.join(opts.output, 'Test.npz')
    np.savez(modelDir,power_trace=pca_traces2,plain_text=text_in2,key=key)
